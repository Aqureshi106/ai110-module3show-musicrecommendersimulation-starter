...
def _score_song_dict(user_prefs: Dict, song: Dict) -> Tuple[float, List[str]]:
    ...
    energy_score = max(0.0, 3.0 - (energy_gap * 6.0))
    score += energy_score
    reasons.append(f"energy is close to your target ({song_energy:.2f})")

    tempo_gap = abs(song_tempo - target_tempo)
    tempo_score = max(0.0, 1.5 - (tempo_gap / 40.0))
    score += tempo_score

    valence_gap = abs(song_valence - target_valence)
    valence_score = max(0.0, 1.25 - (valence_gap * 2.5))
    score += valence_score

    danceability_gap = abs(song_danceability - target_danceability)
    danceability_score = max(0.0, 1.0 - (danceability_gap * 2.0))
    score += danceability_score

    acoustic_bonus = _acoustic_bonus(song_acousticness, likes_acoustic)
...
class Recommender:
    ...
    def _score_song(self, user: UserProfile, song: Song) -> Tuple[float, List[str]]:
        ...
        return _score_song_dict(user_prefs, song_dict)
